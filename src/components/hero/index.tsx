...
export default function index({ stats }: StatProps) {
    return (
        <div>
            <div className="flex flex-col lg:flex-row relative items-center justify-between gap-10 lg:py-0">
                {/* Text Content */}
                <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left lg:pt-10 pt-10 px-30">
                    ...
                    <div className="flex flex-row flex-wrap justify-center lg:justify-start gap-10 lg:gap-20 mt-7">
                        {stats.map((item) => (
                            ...
                            <div key={item.label} className="flex flex-col items-center pb-10 lg:items-start">
                                <span className="text-3xl md:text-3xl font-bold">{item.value.toLocaleString()}+</span>
                                <span className="font-extralight text-sm">{item.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
